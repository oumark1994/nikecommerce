...
const Item = ({ ifExists, id,title,text,img,btn,rating,price,shadow,color}) => {
...
<div className='flex items-center justify-between w-28 my-2'>
    ...
    <button
            ...
            >
             ...
             <button type="button" onClick={()=>{onAddToCart();onCartToggle()}} className='bg-white-90 blur-effect-theme button-theme px-2 py-1 text-sm text-black shadow shadow-sky-200'>
            {btn}
        </button>
    </div>
</div>
<div className={`flex items-center ${ifExists ? 'absolute top-5 right-1':'justify-center'}`}>
    <img src={img} alt={`img/item-img/${id}`} className={` transitions-theme hover:-rotate-12 ${ifExists? 'h-auto w-64 lg:w-56 md:w-48 -rotate-[35deg]':''}`}/>
</div>
