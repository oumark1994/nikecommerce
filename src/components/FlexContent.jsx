...
const FlexContent = ({ifExists, endpoint:{title,heading,subtitle,text,img,btn,url}}) => {
  return (
    ...
    <div className={`flex items-center justify-between lg:flex-col lg:justify-center nike-container ${ifExists ? 'flex-row-reverse':'flex-row'}`}>
        ...
        <a className='flex items-center' target={"_blank"}><button className='button-theme bg-slate-900 shadow-slate-900 text-slate-100 py-1.5' type='button'>{btn}</button></a>
        </div>
